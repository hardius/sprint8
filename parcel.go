...
func (s ParcelStore) Add(p Parcel) (int, error) {
	...
	res, err := s.db.Exec("INSERT INTO parcel(client, status, address, created_at) VALUES (:cl, :st, :addr, :cr_at)",
		sql.Named("cl", p.Client),
		sql.Named("st", p.Status),
		sql.Named("addr", p.Address),
		sql.Named("cr_at", p.CreatedAt))
	if err != nil {
		return 0, err
	}
...
func (s ParcelStore) Delete(number int) error {
	...
	_, err := s.db.Exec("DELETE FROM parcel WHERE number = :number ABD status = :status",
		sql.Named("number", number),
		sql.Named("status", ParcelStatusRegistered))
	if err != nil {
		return err
